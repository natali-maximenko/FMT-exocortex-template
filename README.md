...
2. Заменяет 7 плейсхолдеров (`natali-maximenko`, `/home/natalia/Github` и др.)
...
1. Замените `natali-maximenko` на ваш GitHub username во всех файлах
2. Замените `/home/natalia/Github` на путь к рабочей директории (напр. `~/Github`)
3. Замените `/home/natalia` на домашнюю директорию (значение `$HOME`)
4. Замените `-home-natalia-Github` на путь через дефисы (напр. для `~/Github` → `-Users-yourname-Github`)
